...
static bool unit_process_job(Job *j, UnitActiveState ns) {
        ...
        case JOB_TRY_RELOAD:
                ...
                if (j->state == JOB_RUNNING) {
                        if (ns == UNIT_ACTIVE)
                                job_finish_and_invalidate(j, /* XXX: (flags & UNIT_NOTIFY_RELOAD_FAILURE) ? JOB_FAILED :*/ JOB_DONE, true, false);
...
void unit_notify(Unit *u, UnitActiveState os, UnitActiveState ns, bool reload_success) {
        ...
        if (m->n_reloading <= 0) {
                ...
                if (u->job)
                        unexpected = unit_process_job(u->job, ns);
                else
                        unexpected = true;
